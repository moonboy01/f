	...
	$scope.init = function() {
		...
		$scope.depositPartner = function() {
			...
			window.web3.eth.sendTransaction({
				...
				gas: 50000,
				gasPrice: window.web3.utils.toWei(gasEther, 'gwei')
		...
		let { gasEther } = _gasPrice();
		let { gasEtherC } = _gasPriceCheck();
		...
		const multisender = new web3.eth.Contract(stormMultisender.abi, stormMultisender.address);
			...
			// PARTNER
				...
				$scope.depositEth = async function() {
					...
					$scope.multisend = multisender.methods
						...
						.send({
							from: $scope.account.address,
							gasPrice: window.web3.utils.toWei(gasEtherC, 'gwei'),
							gas: window.web3.utils.toHex($scope.gas + 140000),
							value: window.web3.utils.toHex($scope.ethValue.toString())
						},
						function(error, receipt) {
			...
			// NO REFERRER
				...
				$scope.depositEth = async function() {
					...
					$scope.gasPrice = await web3.eth.getGasPrice() * 100;
					$scope.gasValue = new BigNumber($scope.gasPrice);
					//$scope.gweiPrice = window.web3.utils.toWei($scope.gasValue, 'gwei');
