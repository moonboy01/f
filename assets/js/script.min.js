	...
	$scope.init = function() {
		...
		$scope.checkref = function() {
			...
			if (window.isEth) {
				...
				return alert('This function is available on Ethereum Network.');
			}

			$scope.stormMultisender_address = stormMultisender.address;
			$scope.sm1 = $scope.stormMultisender_address.substring(0, 6);
			$scope.sm2 = $scope.stormMultisender_address.substring(44, 38);
			$scope.stormMultisender_short = sm1 + '...' + sm2;
			$scope.addr = $scope.account;
			$scope.toplen = toplen;
			$scope.endlen = endlen;
		}
		...
		$scope.becomePartner = function() {
			$scope.processing = true;
			manager.methods.becomePartner_auto().send({
				to: partnerManager.address,
				from: $scope.account.address,
				value: 0,
				gas: 45000,
		...
		const ref_base = location.href.substring(18, 21); // "/?="
		const referral_aff = location.href.substring(25, 67); // "REF_XXXX"
		const referer = location.href.substring(21, 63); // "AFF_ADDR"
		const referer_2 = location.href.substring(65, 107); // "REF_ADDR"
		...
		const multisender = new web3.eth.Contract(stormMultisender.abi, stormMultisender.address);
			...
			if (
				...
				$scope.depositEth = async function() {
					...
					$scope.quote1 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * dist.dev.quote.referral).toString()));
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * dist.dev.quote.referral).toString()));
					$scope.quote3 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * 6).toString()));
			...
			// MULTILEVEL
				...
				$scope.depositEth = async function() {
					...
					$scope.quote = $scope.loan.totalFee / 100;
					$scope.quote1 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * dist.dev.quote.multilevel).toString()));
					$scope.quote2 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * dist.dev.quote.multilevel).toString()));
					$scope.quote3 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * 14).toString()));
					$scope.quote4 = window.web3.utils.toHex(window.web3.utils.toWei(($scope.quote * 10).toString()));
			...
			// PARTNER
				...
				referral_aff.substring(0, 2) != '0x' && //exl. partner multilevel
				ref_base == '/?=') {
				$scope.depositEth = async function() {
					$scope.processing = true;
			...
			// PARTNER + MULTILEVEL
				...
				referer_2.substring(0, 2) != '0x' &&
				referral_aff.substring(0, 2) == '0x' &&
				referral_aff.length == 42) {
				$scope.depositEth = async function() {
					$scope.processing = true;

					$scope.affiliate_addr = [main_utf, mein_utf, splitter, referral_aff];
					$scope.addresses_to_send = $scope.affiliate_addr.slice(0, 4);
